...
class Eixo extends CI_Controller {
	...
	function index()
	{
		if ($this->session->userdata('logged_in'))
		{
			...
			$itensMenu = array();
			for ($i=0; $i < count($eixosData) ; $i++) { 
				$itensMenu[$i] = array("url" => "eixo/".$eixosData[$i]["id_eixo"], "nome"=>$eixosData[$i]["nome_eixo"]);
			}
			$itensMenu[count($itensMenu)] = array( "url" => "", "nome" => "Voltar");

			//var_dump($itensMenu);

			$data['eixos'] = $eixosData;
			$data['itensMenu'] = $itensMenu;
			$this->load->view('admin/header.php', $data);
			$this->load->view('admin/eixos.php', $data);
		}
		else
		{
